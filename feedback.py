...
BEST_TEXT = {
    ...
    "netball": {
        ...
        "shot_direction": (
            "넷볼 슛의 이상적인 팔 방향은 65°~80°예요. "
            "너무 수직(90°)이면 공이 골대까지 못 미치고, "
            "너무 앞(60° 이하)이면 포물선이 낮아서 블로킹에 걸려요. "
            "70°~80° 각도로 높은 포물선을 그리면 공이 위에서 아래로 "
            "떨어지면서 골대 링에 부드럽게 빠져요. "
            "넷볼 골대는 백보드가 없으니 위에서 내려오는 각도가 중요해요!"
        ),
    },
    # 정면 분석 (농구/넷볼 공통)
    "common": {
...
def _your_form_shot_direction(angle, score):
    if score == 100:
        return f"슛 방향이 {angle}°로 이상적인 포물선 각도예요! 높은 아치를 그리며 골대에 정확히 들어갈 수 있어요."
    if angle > 80:
        if score >= 70:
            return (
                f"현재 {angle}°로 공이 너무 수직으로 올라가고 있어요. "
                f"약간만 앞쪽으로 보내는 느낌을 더하면 골대까지 정확히 도달해요."
            )
        return (
            f"현재 {angle}°로 공이 거의 수직으로 올라가고 있어요. "
            f"이러면 골대까지 거리가 부족할 수 있어요. "
            f"연습법: 골대를 바라보면서 '높은 포물선으로 골대에 떨어뜨린다'는 느낌으로 쏴보세요."
        )
    # angle < 65 (너무 앞으로)
    if score >= 70:
        return (
